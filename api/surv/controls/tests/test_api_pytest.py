...
def test_create_control_without_data_should_fail(client)->None:
    ...
    assert json.loads(response.content) == {"name": ["This field is required."],"cid": ["This field is required."]}
...
def test_create_existing_controls_should_fails(client)-> None:
    ...
    assert response.status_code == 400
    assert json.loads(response.content)== {"name": ["control with this name already exists."], "cid": ["control with this cid already exists."]}  


def test_create_control_with_only_name_and_id_all_fields_should_be_default(client)->None:
...
def test_create_control_with_ctype_primary_should_succeed(client)->None:
    response = client.post(path=controls_url, data={"name":"muhoroni1","cid":"mhn1","ctype":"Primary"})
    response_content = json.loads(response.content)
    assert response.status_code == 201
    assert response_content.get("name") == "muhoroni1"
    assert response_content.get("cid") == "mhn1"
